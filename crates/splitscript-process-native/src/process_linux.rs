//! Linux-specific module size resolution for PE images under Wine / Proton.
...
pub(super) fn module_size(process: &Process, module_address: u64, mapped_size: u64) -> u64 {
    logical_module_size(module_address, mapped_size, |address, buffer| {
        ...
        process.handle.copy_address(address, buffer)
    })
}
...
fn read_pe_size_of_image(
    ...
    let minimum_optional_header_size = match read_u16(&optional_header, 0)? {
        PE32_MAGIC => 0x60,
...
mod tests {
    ...
    const BASE_ADDRESS: u64 = 0x0001_4000_0000;
    ...
    fn module_size(image: &[u8]) -> u64 {
        logical_module_size(BASE_ADDRESS, MAPPED_SIZE, |address, buffer| {
            let offset = address
