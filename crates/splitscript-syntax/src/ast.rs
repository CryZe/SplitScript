...
pub struct ManagedClassDecl {
    ...
    /// Stable identity of the implicit read-only `<Class>.layout` value.
...
pub struct ManagedLayoutDecl {
    ...
    /// The corresponding variant in the generated `<Class>.Layout` enum.
...
impl Program {
    /// Iterates ordinary source enums and enums generated by domain-specific
    /// declarations such as state and managed-class layouts.
    pub fn enum_declarations(&self) -> impl Iterator<Item = &EnumDecl> {
        let managed = self
            .managed_class_declarations()
            .into_iter()
            .filter_map(|class| class.layout_enum.as_ref())
            .collect::<Vec<_>>();
        self.enums
            .iter()
            .chain(
