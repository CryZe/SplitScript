...
| `asl.state.memory-watcher` — MemoryWatcher | ASL | Use a typed pattern | Declare polled memory in `state`; `old` and `current` expose transactional snapshots. Canonical targets: `state`. |
...
| `asl.settings.dynamic-lookup` — Dynamic settings lookup | ASL | Planned | Stable dynamic setting keys and typed runtime lookup are not implemented yet. Canonical targets: `settings`. |
