...
const VOID: TypeRef = TypeRef::Builtin(BuiltinType::Void);
const U32: TypeRef = TypeRef::Builtin(BuiltinType::U32);
const U64: TypeRef = TypeRef::Builtin(BuiltinType::U64);
const I32: TypeRef = TypeRef::Builtin(BuiltinType::I32);
const I64: TypeRef = TypeRef::Builtin(BuiltinType::I64);
const F32: TypeRef = TypeRef::Builtin(BuiltinType::F32);
const F64: TypeRef = TypeRef::Builtin(BuiltinType::F64);
const ADDRESS: TypeRef = TypeRef::Builtin(BuiltinType::Address);
const STRING: TypeRef = TypeRef::Builtin(BuiltinType::String);
const SIGNATURE: TypeRef = TypeRef::Builtin(BuiltinType::Signature);
const DURATION: TypeRef = TypeRef::Builtin(BuiltinType::Duration);
const MODULE: TypeRef = TypeRef::Builtin(BuiltinType::Module);
const UNITY_MODULE: TypeRef = TypeRef::Builtin(BuiltinType::UnityModule);
const UNITY_IMAGE: TypeRef = TypeRef::Builtin(BuiltinType::UnityImage);
const UNITY_CLASS: TypeRef = TypeRef::Builtin(BuiltinType::UnityClass);
const UNITY_FIELD: TypeRef = TypeRef::Builtin(BuiltinType::UnityField);
const TIMER_STATE: TypeRef = TypeRef::Named(TIMER_STATE_TYPE_NAME);
...
macro_rules! function_item {
    ($id:ident, $qualified:literal, [$($path:literal),+], $params:expr, $result:expr,
     $effects:expr, $availability:expr, $summary:literal, $details:literal, $examples:expr) => {
        StdlibItem {
            id: StdlibItemId::$id,
            qualified_name: $qualified,
            kind: ItemKind::Function { path: &[$($path),+] },
            signature: Signature {
                type_parameters: &[],
                parameters: $params,
                result: $result,
    ...
    ($id:ident => $intrinsic:ident, $qualified:literal, [$($path:literal),+], $params:expr,
...
macro_rules! method_item {
    ($id:ident, $qualified:literal, $receiver:expr, $name:literal, $types:expr,
...
impl StandardLibrary {
    ...
    pub fn function_candidates(self, path: &[String]) -> Vec<CallCandidate> {
        ITEMS
            ...
            .filter_map(|item| match item.kind {
                ItemKind::Function { path: declared }
                ...
                ItemKind::TypedFunction {
                    prefix,
                    type_parameter,
                } if (path.len() == prefix.len() || path.len() == prefix.len() + 1)
                    && path[..prefix.len()]
                        .iter()
                        .map(String::as_str)
                        .eq(prefix.iter().copied()) =>
                {
                    Some(CallCandidate {
                        item,
                        type_arguments: if path.len() == prefix.len() {
                            Vec::new()
                        } else {
                            vec![(type_parameter, memory_type(&path[prefix.len()])?)]
                        },
                    })
                }
                ItemKind::Function { .. }
    ...
    pub fn method_candidates(self, name: &str) -> Vec<CallCandidate> {
        ITEMS
            .iter()
            .filter_map(|item| {
                matches!(item.kind, ItemKind::Method { name: declared, .. } if declared == name)
                    .then_some(CallCandidate {
                        item,
                        type_arguments: Vec::new(),
                    })
            })
    ...
    pub fn render_signature_with(
        ...
        let mut rendered = match item.kind {
            ...
            ItemKind::Method { receiver, name } => {
                format!("{}.{}(", receiver.render_with(substitutions), name)
            }
        };
    ...
    pub fn validate(self) -> Vec<String> {
        ...
        for item in ITEMS {
            ...
            let call_shape = match item.kind {
                ItemKind::Function { path } => format!("function {}", path.join(".")),
                ItemKind::TypedFunction { prefix, .. } => {
                    format!("typed function {}[.*]", prefix.join("."))
                }
                ItemKind::Method { receiver, name } => {
                    format!("method {}.{name}", receiver.render())
                }
            };
            ...
            let example_call = match item.kind {
                ItemKind::Function { path } => format!("{}(", path.join(".")),
                ItemKind::TypedFunction { prefix, .. } => format!("{}.", prefix.join(".")),
                ItemKind::Method { name, .. } => format!(".{name}("),
            };
