...
pub enum ResolvedTypeRef {
    ...
    ManagedReference(ManagedClassId),
...
pub enum TypeKind {
    ...
    Struct(StructId),
    Enum(EnumId),
    /// Immutable local snapshot shape generated by a managed class schema.
    ManagedClass(ManagedClassId),
    /// Live remote reference to one managed class schema. The backend stores
    /// it as a process address, but it remains nominally distinct from both
    /// `address` and the class's local snapshot type.
    ManagedReference(ManagedClassId),
    GenericParameter {
        owner: FunctionId,
        index: u32,
...
impl TypeStore {
    ...
    pub(crate) fn with_source_types(
        ...
        for structure in structs {
            store.intern(TypeKind::Struct(structure.id));
        }
        for enumeration in enums {
            store.intern(TypeKind::Enum(enumeration.id));
        }
        for class in managed_classes {
            store.intern(TypeKind::ManagedClass(class.id));
            store.intern(TypeKind::ManagedReference(class.id));
        }
        store
    }
    ...
    pub fn id_for_struct(&self, structure: StructId) -> TypeId {
        self.interned[&TypeKind::Struct(structure)]
    }
    ...
    pub fn id_for_managed_class(&self, class: ManagedClassId) -> TypeId {
        self.interned[&TypeKind::ManagedClass(class)]
    }

    pub fn id_for_managed_reference(&self, class: ManagedClassId) -> TypeId {
        self.interned[&TypeKind::ManagedReference(class)]
    }
    ...
    fn intern_type_ref(
        ...
        match ty {
            ...
            ResolvedTypeRef::Struct(structure) => self.id_for_struct(structure),
            ResolvedTypeRef::Enum(enumeration) => self.id_for_enum(enumeration),
            ResolvedTypeRef::ManagedClass(class) => self.id_for_managed_class(class),
            ResolvedTypeRef::ManagedReference(class) => self.id_for_managed_reference(class),
