...
if (!wasmPath) {
    throw new Error("usage: node tests/map_runtime.mjs <map.wasm>");
}
...
const observed = {
    first: variables.get("Map First Tick"),
    debug: variables.get("Map Debug"),
    second: variables.get("Map Second Tick"),
    messages,
};
const expected = {
    first: "true,true,false,3,16,true",
    debug: "Map {\n    \"Atrium\": 16,\n    \"Vault\": 20,\n    \"Tower\": 30,\n}",
    second: "true,false,0,true",
    messages: ["Atrium:16", "Vault:20", "Tower:30"],
};
if (JSON.stringify(observed) !== JSON.stringify(expected)) {
    throw new Error(`unexpected map output: ${JSON.stringify({ expected, observed })}`);
}
