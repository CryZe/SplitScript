...
if (!wasmPath) {
    throw new Error("usage: node tests/wrapper_equality_runtime.mjs <equality.wasm>");
}
...
const env = {
    ...
    timer_set_variable(keyPointer, keyLength, valuePointer, valueLength) {
        if (text(keyPointer, keyLength) === "Wrapper Equality") {
...
if (observed !== expected) {
    throw new Error(`unexpected wrapper equality score: ${JSON.stringify({ expected, observed })}`);
}
